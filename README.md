...
| BLAS f32 layer (blas/src/f32/, the tuned f64 layer cloned one-for-one): 23 fns, 30 mirrored tests (same-order replays bit-for-bit, reduction bounds vs an f64-accumulated reference, nrm2 guards at 1e±30), 21 probes bit-identical native ↔ wasm on container + both runner draws | tested | scripted | `blas/tests/level{1,2,3}_f32.rs`; `run_l{1,2,3}_probe_f32` vs `native l{1,2,3}-bits-f32` |
