...
| BLAS layer Level 1 (blas/ crate, 10 f64 functions): elementwise streams bit-for-bit vs the scalar definition, reductions error-bounded vs compensated reference, iamax index exact, nrm2 over/underflow guards exercised — 12 tests | tested | scripted | `blas/tests/L1/` (CI gate line pending — workflow edit needs Andy's token) |
...
| fused single-pass iamax REFUTED by the same draws (1.39×/1.60× copy's time vs 1.35/1.36 two-pass) — reverted; negative result recorded in the module docs | observed | scripted | docs step 9; `blas/src/L1/idamax.rs` |
| BLAS f32 layer (the s-prefixed routines, the tuned f64 layer cloned one-for-one): 23 routines, 30 mirrored tests (same-order replays bit-for-bit, reduction bounds vs an f64-accumulated reference, nrm2 guards at 1e±30), 21 probes bit-identical native ↔ wasm on container + both runner draws | tested | scripted | `blas/tests/L{1,2,3}/` s-files; `run_l{1,2,3}_probe_f32` vs `native l{1,2,3}-bits-f32` |
| f32 rooflines on runners (2 draws): arithmetic peak 26.6–27.7 GFLOP/s (~1.8× f64), L3 family 48–58% of it (symm_left 79–82%) — the tuned shapes transferred at double lane width without re-tuning; reductions at the read path; f32 gemm dispatch crossover runner-raced to 3 MB of A (tiled unanimous ≤512, col4 unanimous at 1024; container said the opposite and was overruled) | observed | scripted | `bench/l{1,2,3}-roofline.mjs --f32`, runs 29676350376/29676348498; docs step 10 |
| f32 iamax at ~8% of ceiling both draws (f64: 17–23%) — the scalar first-index rescan is per-element and dominates at n=2048²; recorded lever, not chased | observed | scripted | docs step 10 |
| BLAS c64 layer (the z-routines, first non-mechanical clone): 26 routines / 31 operations over the crate's own repr(C) `C64` with a fixed rounding order; the SIMD complex product (sign-folded two-multiply lane form) proven bit-exact to the scalar order by 40 new tests — same-order replays bit-for-bit incl. the grouped reorders, conjugation cross-checks bitwise (zdotc==zdotu(conj x), zgemv_c==zgemv_t(conj A)), Hermitian diagonals exactly real; 24 probes bit-identical native ↔ wasm on container + both runner draws | tested | scripted | `blas/tests/L{1,2,3}/` z-files; `run_l{1,2,3}_probe_z` vs `native l{1,2,3}-bits-z` |
...
| packed-panel gemm (all four types) bit-identical to the colaxpy references — per-element accumulation order unchanged through packing and k-blocking (β once, ascending k, resumed from stored partials) | tested | scripted | replay tests across KC/MC/MR/tail boundaries (`blas/tests/L3/*gemm.rs`, 52 L3 tests) + wasm fold identity per type/shape on both race draws |
| packed-gemm race, two draws, same-shape ratios within ~3%: dgemm packed 1.24×@512³ / 1.44×@1024³ / 1.10–1.23× deep-K (unanimous — replaces col4 above 1.5 MB); sgemm +3–5% above 3 MB only; zgemm 1.08–1.33× at every measured size (packed at A ≥ 1 MB, ships the complex register tile); cgemm +12%@1024³ only (packed at A ≥ 8 MB); win gradient tracks memory-boundedness (f64 most → c32 least) | observed | scripted | `blas/bench/packed-gemm-ab.mjs`, runs 29720778259/29720782633; docs step 14 |
