//! `dsyr2k` — symmetric rank-2k update: C ← α(ABᵀ + BAᵀ) + βC, one
//! triangle of C stored.
//!
//! Implementation: truncated column-daxpy, 4-column fan-out (tuned
//! 2026-07-19, same shape as dsyrk) — four stored columns grouped so
...
pub fn dsyr2k(
	...
	while j < n {
		...
		dscale_y(beta, seg);
		for l in 0..k {
			daxpy(alpha * b[l * bcs + j], &a[l * acs + lo..l * acs + hi], seg);
			daxpy(alpha * a[l * acs + j], &b[l * bcs + lo..l * bcs + hi], seg);
		}
		j += 1;
	}
}
