//! `dsymv` — symmetric matrix × vector: y ← αAx + βy, A symmetric with
...
//! triangles' contributions for all four (`kernels::daxpy_dot4`; the
//! ≤3-row ragged edge and diagonals scalar per column), where the
//! plain shape streamed each column twice (`daxpy` + `ddot`). dsymv has
...
//! single-column fused pass (`kernels::daxpy_dot`). Accumulation
//! order differs from reference `dsymv` (dsymv is bounds-tested, not
//! bit-locked; cross-target determinism holds through the lane
//! emulation as everywhere else).

use super::{check_mat, dscale_y};
use crate::kernels::{daxpy_dot, daxpy_dot4};
...
pub fn dsymv(
	...
	assert_eq!(x.len(), n, "dsymv: x length mismatch");
	assert_eq!(y.len(), n, "dsymv: y length mismatch");
	dscale_y(beta, y);
	let ap = a.as_ptr();
	let mut j = 0usize;
	while j + 4 <= n {
		...
		if upper {
			...
			let mut d =
				unsafe { daxpy_dot4(cols, t, x.as_ptr(), y.as_mut_ptr(), j) };
			for u in 0..4 {
				let cj = (j + u) * cs;
				for i in j..j + u {
					y[i] += t[u] * a[cj + i];
	...
	while j < n {
		...
		let t = alpha * x[j];
		if upper {
			let d = unsafe { daxpy_dot(col.as_ptr(), t, x.as_ptr(), y.as_mut_ptr(), j) };
			y[j] += t * col[j] + alpha * d;
		} else {
			let d = unsafe {
				daxpy_dot(
