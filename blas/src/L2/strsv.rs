//! `strsv` — triangular solve, one right-hand side, in place: x ← A⁻¹x.
//!
//! Implementation: divide-then-column-saxpy, 4-column fan-in (tuned
...
//! pass (`kernels::saxpy4in`, sources in the same step order), cutting
...
pub fn strsv(n: usize, a: &[f32], cs: usize, upper: bool, unit: bool, x: &mut [f32]) {
	check_mat(a.len(), n, n, cs);
	assert_eq!(x.len(), n, "strsv: x length mismatch");
	let ap = a.as_ptr();
	if upper {
		let r = n % 4;
