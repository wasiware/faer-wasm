...
//! reference, which is kept as `gemm_colaxpy`. Shapes ported from the
//! raced f64 layer (both beat faer's blocked gemm at every measured
//! f64 size — docs/blas-ab-2026-07.md step 6); f32 measurements:
//! step 10. Transpose forms: not built — no consumer yet (syrk covers
//! A·Aᵀ).

use super::check_mat;
use crate::f32::kernels::axpy4;
use crate::f32::level2::gemv;
...
/// Tuning-campaign shape (ported from f64): 8×4 register-tiled gemm.
	...
	while j + 4 <= n {
		...
		if i < m {
			for jj in j..j + 4 {
				...
				crate::f32::level2::scale_y(beta, seg);
				for l in 0..k {
					crate::f32::level1::axpy(
						alpha * b[jj * bcs + l],
						&a[l * acs + i..l * acs + m],
						seg,
					);
	...
	while j + 4 <= n {
		{
			...
			for u in 0..4 {
				...
				crate::f32::level2::scale_y(beta, col);
			}
			for l in 0..k {
				let t = [
					alpha * b[j * bcs + l],
