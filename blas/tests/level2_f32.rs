...
use faer_wasm_blas::f32::level1::asum;
use faer_wasm_blas::f32::level2::*;

struct Lcg(u64);
impl Lcg {
	fn next_f64(&mut self) -> f32 {
		self.0 = self.0.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
		let bits = ((self.0 >> 11) as f64 / (1u64 << 53) as f64) as f32;
		4.0 * bits - 2.0
	}
	...
	fn mat(&mut self, nrows: usize, ncols: usize, cs: usize) -> Vec<f32> {
		assert!(cs >= nrows);
...
// them), accumulated exactly in f32 — the higher-precision reference.
fn comp_sum(it: impl Iterator<Item = f32>) -> f64 {
	let mut s = 0.0f64;
	for v in it {
		s += v as f64;
	}
		...
		for &pad in PADS {
			...
			for (alpha, beta) in [(1.0, 0.0), (-0.5, 1.0), (0.3, -2.0), (0.0, 0.5)] {
				...
				for i in 0..m {
					...
					assert!((y[i] as f64 - want).abs() <= tol, "gemv bound {m}x{n} i={i}");
				}
			}
		}
	}
...
fn gemv_t_bounded() {
	...
	for &(m, n) in SHAPES {
		...
		for j in 0..n {
			let want = 0.7 * comp_sum((0..m).map(|i| a[j * cs + i] * x[i])) - 0.3 * y0[j] as f64;
			let scale = comp_sum((0..m).map(|i| (a[j * cs + i] * x[i]).abs())) + y0[j].abs() as f64;
			let tol = f32::EPSILON as f64 * (m.max(1) as f64) * 4.0 * scale + 1e-40;
			assert!((y[j] as f64 - want).abs() <= tol, "gemv_t {m}x{n} j={j}");
		}
	}
}
...
fn symv_bounded_both_triangles() {
	...
	for &n in NS {
		for upper in [true, false] {
			...
			let mut a = vec![f32::NAN; if n == 0 { 0 } else { cs * (n - 1) + n }];
			for j in 0..n {
				for i in 0..n {
			...
			for i in 0..n {
				let want =
					0.9 * comp_sum((0..n).map(|j| full[j * n + i] * x[j])) + (0.4 * y0[i]) as f64;
				let scale =
					comp_sum((0..n).map(|j| (full[j * n + i] * x[j]).abs())) + y0[i].abs() as f64;
				let tol = f32::EPSILON as f64 * (n.max(1) as f64) * 4.0 * scale + 1e-40;
				assert!((y[i] as f64 - want).abs() <= tol, "symv upper={upper} n={n} i={i}");
			}
		}
	}
}
		...
		for upper in [true, false] {
			for unit in [true, false] {
				...
				for i in 0..n {
					...
					let scale = asum(&x) as f64 * (2.0 * n as f64 + n as f64) + b[i].abs() as f64;
