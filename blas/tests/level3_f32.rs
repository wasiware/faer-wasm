...
fn gemm_bit_for_bit_and_bounded() {
	...
	for &(m, k, n) in DIMS {
		...
		for (alpha, beta) in [(1.0, 0.0), (-0.7, 0.4)] {
			...
			for j in 0..n {
				for i in 0..m {
					let want = alpha as f64 * comp_sum((0..k).map(|l| a[l * acs + i] * b[j * bcs + l]))
					...
					assert!((c[j * ccs + i] as f64 - want).abs() <= tol, "gemm bound ({i},{j})");
				}
			}
		}
	}
		...
		for upper in [true, false] {
			...
			for j in 0..n {
				for i in 0..m {
					...
					assert!((c[j * ccs + i] as f64 - want).abs() <= tol, "symm_left ({i},{j})");
				}
			}
			...
			for j in 0..n {
				for i in 0..m {
					...
					let scale = comp_sum((0..n).map(|l| (b[l * bcs + i] * full[j * n + l]).abs()))
						+ c0[j * ccs + i].abs() as f64;
					let tol = f32::EPSILON as f64 * (n.max(1) as f64) * 8.0 * scale + 1e-40;
					assert!((c[j * ccs + i] as f64 - want).abs() <= tol, "symm_right ({i},{j})");
				}
			}
