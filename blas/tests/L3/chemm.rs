...
#[test]
fn chemm_both_sides_bounded_right_bit_replay() {
	let mut rng = Lcg(132);
	for &n in NS {
		...
		for upper in [true, false] {
			...
			let (bcs, ccs) = (m + 2, m + 1);
			let b = rng.mat_c32(m, n, bcs);
			let c0 = rng.mat_c32(m, n, ccs);
			let alpha = C32::new(0.8, -0.3);
			let beta = C32::new(0.3, 0.5);

			let mut c = c0.clone();
			chemm_left(alpha, m, n, &a, acs, upper, &b, bcs, beta, &mut c, ccs);
			for j in 0..n {
				for i in 0..m {
					let want = c_up(alpha)
						* comp_sum_cc((0..n).map(|l| full[l * n + i] * b[j * bcs + l]))
						+ c_up(beta * c0[j * ccs + i]);
					let scale = comp_scale_cc((0..n).map(|l| full[l * n + i] * b[j * bcs + l]))
						* (alpha.abs1() as f64 + 1.0)
						+ (beta * c0[j * ccs + i]).abs1() as f64;
					let tol = EPS * (n.max(1) as f64) * 16.0 * scale + 1e-30;
					assert!(
						(c[j * ccs + i].re as f64 - want.re).abs()
							+ (c[j * ccs + i].im as f64 - want.im).abs()
							<= tol,
						"chemm_left ({i},{j})"
					);
				}
			...
			let mut c = c0.clone();
			chemm_right(alpha, m, n, &a, acs, upper, &b, bcs, beta, &mut c, ccs);
			for j in 0..n {
				for i in 0..m {
					let want = c_up(alpha)
						* comp_sum_cc((0..n).map(|l| b[l * bcs + i] * full[j * n + l]))
						+ c_up(beta * c0[j * ccs + i]);
					let scale = comp_scale_cc((0..n).map(|l| b[l * bcs + i] * full[j * n + l]))
						* (alpha.abs1() as f64 + 1.0)
						+ (beta * c0[j * ccs + i]).abs1() as f64;
					let tol = EPS * (n.max(1) as f64) * 16.0 * scale + 1e-30;
					assert!(
