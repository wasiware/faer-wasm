	...
	for &n in NS {
		...
		for upper in [true, false] {
			for unit in [true, false] {
				let acs = n + 1;
				let mut a = rng.mat_f64(n, n, acs);
				for j in 0..n {
					a[j * acs + j] = 2.0 * (n as f64) + 1.0 + j as f64;
				}
				let bcs = m + 1;
				let b0 = rng.mat_f64(m, n, bcs);
				...
				// left: A·X = α·B0
				let mut x = b0.clone();
				dtrsm_left(0.9, m, n, &a, acs, upper, unit, &mut x, bcs);
				for j in 0..n {
					for i in 0..m {
						...
						assert!(
							(ax - want).abs() <= tol,
							"dtrsm_left residual upper={upper} unit={unit} n={n} ({i},{j})"
						);
					}
				}
				...
				// right: X·A = α·B0
				let mut x = b0.clone();
				dtrsm_right(0.9, m, n, &a, acs, upper, unit, &mut x, bcs);
				for j in 0..n {
					for i in 0..m {
						...
						assert!(
							(xa - want).abs() <= tol,
							"dtrsm_right residual upper={upper} unit={unit} n={n} ({i},{j})"
						);
					}
				}
			}
	...
	for &(m, n) in TRI_DIMS {
		for upper in [true, false] {
			for unit in [true, false] {
				...
				// left: B <- alpha * inv(A) * B, plain scal-then-dtrsv replay
				let acs = m + 1;
				let mut a = rng.mat_f64(m, m, acs);
				for j in 0..m {
					a[j * acs + j] = 2.0 * (m as f64) + 1.0 + j as f64;
				}
				let b0 = rng.mat_f64(m, n, bcs);
				let mut bt = b0.clone();
				dtrsm_left(0.9, m, n, &a, acs, upper, unit, &mut bt, bcs);
				let mut br = b0.clone();
				for j in 0..n {
					for i in 0..m {
						br[j * bcs + i] *= 0.9;
				...
				for (x, y) in bt.iter().zip(&br) {
					if !x.is_nan() {
						assert_eq!(x.to_bits(), y.to_bits(), "dtrsm_left {m}x{n} u={upper}");
					}
				}
				...
				let b0 = rng.mat_f64(m, n, bcs);
				let mut bt = b0.clone();
				dtrsm_right(0.9, m, n, &a, acs, upper, unit, &mut bt, bcs);
				let mut br = b0.clone();
				...
				for (x, y) in bt.iter().zip(&br) {
					if !x.is_nan() {
						assert_eq!(x.to_bits(), y.to_bits(), "dtrsm_right {m}x{n} u={upper}");
					}
				}
			}
		}
