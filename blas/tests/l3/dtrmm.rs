	...
	for &n in NS {
		...
		for upper in [true, false] {
			for unit in [true, false] {
				...
				let mut b = b0.clone();
				dtrmm_left(0.9, m, n, &a, acs, upper, unit, &mut b, bcs);
				for j in 0..n {
					for i in 0..m {
						...
						assert!(
							...
							"dtrmm_left upper={upper} unit={unit} n={n} ({i},{j})"
						);
					}
				}

				let mut b = b0.clone();
				dtrmm_right(0.9, m, n, &a, acs, upper, unit, &mut b, bcs);
				for j in 0..n {
					for i in 0..m {
						...
						assert!(
							(b[j * bcs + i] - want).abs() <= tol,
							"dtrmm_right upper={upper} unit={unit} n={n} ({i},{j})"
						);
					}
				}
			}
	...
	for &(m, n) in TRI_DIMS {
		for upper in [true, false] {
			for unit in [true, false] {
				...
				// left: B <- alpha * A * B, plain dtrmv-per-column replay
				let acs = m + 1;
				let a = rng.mat_f64(m, m, acs);
				let b0 = rng.mat_f64(m, n, bcs);
				let mut bt = b0.clone();
				dtrmm_left(0.9, m, n, &a, acs, upper, unit, &mut bt, bcs);
				let mut br = b0.clone();
				for j in 0..n {
					if upper {
						for l in 0..m {
				...
				for (x, y) in bt.iter().zip(&br) {
					if !x.is_nan() {
						assert_eq!(x.to_bits(), y.to_bits(), "dtrmm_left {m}x{n} u={upper}");
					}
				}
				...
				let b0 = rng.mat_f64(m, n, bcs);
				let mut bt = b0.clone();
				dtrmm_right(0.9, m, n, &a, acs, upper, unit, &mut bt, bcs);
				let mut br = b0.clone();
				...
				for (x, y) in bt.iter().zip(&br) {
					if !x.is_nan() {
						assert_eq!(x.to_bits(), y.to_bits(), "dtrmm_right {m}x{n} u={upper}");
					}
				}
			}
		}
