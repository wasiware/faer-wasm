...
/// Interleaved A/B rows for the complex fused-hemv grouping race
/// (close-out campaign): op 0 = zhemv (shipped single-column pass),
/// 1 = zhemv_grouped (4-column candidate), 2 = chemv, 3 =
/// chemv_grouped. Same state, constants, and upper=true as the
/// run_l2_layer_{z,c} hemv rows.
#[no_mangle]
pub extern "C" fn run_hemv_ab(op: usize) -> f64 {
    use faer_wasm_blas::L2 as l2;
    let s = state_mut();
    let n = s.n;
    let cs = s.n;
    ...
    match op {
        0 | 1 => {
            let sym = unsafe { core::slice::from_raw_parts(s.symz.as_ptr(), len) };
            let bx = unsafe { core::slice::from_raw_parts(s.bz.as_ptr(), n) };
            let y = unsafe { core::slice::from_raw_parts_mut(s.rhsz.as_mut_ptr(), n) };
            let al = C64::new(0.001, 0.0);
            let be = C64::new(0.5, 0.0);
            if op == 0 {
                l2::zhemv(al, n, sym, cs, true, bx, be, y);
            } else {
                l2::zhemv_grouped(al, n, sym, cs, true, bx, be, y);
            }
            y[0].re + y[n - 1].im
        }
        2 | 3 => {
            let sym = unsafe { core::slice::from_raw_parts(s.symc.as_ptr(), len) };
            let bx = unsafe { core::slice::from_raw_parts(s.bc.as_ptr(), n) };
            let y = unsafe { core::slice::from_raw_parts_mut(s.rhsc.as_mut_ptr(), n) };
            let al = C32::new(0.001, 0.0);
            let be = C32::new(0.5, 0.0);
            if op == 2 {
                l2::chemv(al, n, sym, cs, true, bx, be, y);
            } else {
                l2::chemv_grouped(al, n, sym, cs, true, bx, be, y);
            }
            (y[0].re + y[n - 1].im) as f64
        }
        _ => f64::NAN,
    }
}
